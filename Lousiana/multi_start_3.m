...
LowerBounds = [0.5       0.1     0.0001       0.23         0.08    0.0001];
UpperBounds = [3         0.4      0.02        0.32          0.12    0.02];

 xstar=0.5*(LowerBounds + UpperBounds); % initial guesses for parameters
 
% % % % %  Optimization Function fmincon  % % % % %
problem=createOptimProblem('fmincon','x0',xstar, 'objective',@model_ode45_3...
...
global ModelParamaters3

for i=1: length(manymins) 
    ModelParamaters3(i,:)=manymins(i).X; %% model parameters
end
